...
const markOverdueAssignments = `-- name: MarkOverdueAssignments :execrows
UPDATE assignments
SET status = 'missed'
...
func (q *Queries) MarkOverdueAssignments(ctx context.Context, dueAt *time.Time) (int64, error) {
	result, err := q.db.Exec(ctx, markOverdueAssignments, dueAt)
	if err != nil {
		return 0, err
	}
