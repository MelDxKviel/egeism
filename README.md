...
```bash
# 0. Первый запуск API создаёт админа (ADMIN_USERNAME/ADMIN_PASSWORD;
#    dev-compose ставит admin/admin). Самостоятельной регистрации нет.
ADM=$(curl -s localhost:8080/api/auth/login \
  -d '{"username":"admin","password":"admin"}' | jq -r .token)

# 1. Админ создаёт аккаунт ученика → ученик логинится
curl -s localhost:8080/api/admin/users -H "Authorization: Bearer $ADM" \
  -d '{"role":"student","username":"anya","password":"secret1","name":"Аня"}'
TOKEN=$(curl -s localhost:8080/api/auth/login \
  -d '{"username":"anya","password":"secret1"}' | jq -r .token)
...
Авторизация: **JWT-сессии**, роли **админ / учитель / ученик** (привязаны к
аккаунту, без тумблера), бот — на токенах. Самостоятельной регистрации нет:
аккаунты создаёт админ (панель «Пользователи») или учитель (своих учеников).
Учителя бывают предметными (`users.subject`) и сверхучителями (все предметы);
ученики группируются в **классы** (или занимаются индивидуально), у класса —
цветовая сетка успеваемости по номерам заданий.
